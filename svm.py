...
def rbf(x_1, x_2, sigma=1.):
    ...
    '''
    samples_1 = x_1.size(0)
    samples_2 = x_2.size(0)
    features = x_1.size(1)
    x_1 = x_1.unsqueeze(1).expand(samples_1, samples_2, features)
    x_2 = x_2.unsqueeze(0).expand(samples_1, samples_2, features)

    distances = torch.exp((torch.pow(x_1 - x_2, 2).sum(2) * torch.pow(x_1 - x_2, 2).sum(2))/ (-2 * sigma * sigma)) ### YOUR CODE HERE
...
def hinge_loss(scores, labels):
    ...
    new_tensor = 1 - torch.mul(scores, labels)
    return torch.mean(new_tensor[new_tensor > 0]) ### YOUR CODE HERE
...
class SVM(BaseEstimator, ClassifierMixin):
    ...
    def linear(x_1, x_2):
        ...
        '''
        return  torch.Tensor(x_1.t() @ x_2)### YOUR CODE HERE
    ...
    def predict_scores(self, batch):
        with torch.no_grad():
            ...
            return  ( K @ self.betas - self.bias).flatten().cpu().detach().numpy()### YOUR CODE HERE
